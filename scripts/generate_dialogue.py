...
def generate_image(text, font_dir, out_path, space_width=12, line_spacing=8):
...
def main(argv):
    ...
    parser.add_argument('--font-dir', default=None, help='Path to custom_font folder (defaults to repo custom_font)')
    ...
    font_dir = args.font_dir if args.font_dir else os.path.join(repo_root, 'custom_font')
    out = args.output
    if not out:
        ts = datetime.now().strftime('%Y%m%d_%H%M%S')
        out = os.path.join(repo_root, 'dialogue', f'dialogue_{ts}.png')
    out_path = generate_image(text_arg, font_dir, out)
